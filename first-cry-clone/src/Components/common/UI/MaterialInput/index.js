...
export default function MaterialInput({
  ...
  switch (elemConfig.type) {
        ...
        >
          <div className="form-item">
            <Select
              ...
              placeholder=""
              className={`${className} ${
                !isEmpty(elemConfig.value) ? "has-value" : ""
              } ${isFocus ? " focused" : ""} item-select`}
          ...
          {error && (
            <ErrorMessage error={errorMsg} datetimestamp={+new Date()} />
          )}
        </MaterialInputWrapper>
      );
    case "date":
      return (
        ...
        >
          <div
            className={`${isCalenarOpen ? "focused" : ""} ${
              elemConfig?.value ? "has-value" : ""
            } form-item form-date`}
          >
            <DatePicker
              {...elemConfig}
    ...
    case "textarea":
      return (
        <MaterialInputWrapper
          className={`${wrapperClassName} ${!icon ? "without-icon" : ""}`}
        >
          <div className="form-item">
            <textarea
              {...elemConfig}
              className={`${className} item-textarea`}
            />
            <label className="item-label-wrapper">
              {icon && <span className="item-icon">{icon}</span>}
              {placeholder && <span className="item-label">{placeholder}</span>}
    ...
    case "checkbox":
      return (
        <CheckFieldWrapper
          className={`${wrapperClassName} flex flex-center space-between checkboxinputbox`}
        >
    ...
    case "radio":
      return (
        <CheckFieldWrapper
          className={`${wrapperClassName} flex flex-center space-between`}
        >
          <label className="form-item">
            <input {...elemConfig} />
            <Text size="rg" color="color3">
    ...
    case "file":
      return (
        ...
        >
          ...
          {uploadResponse?.msg && uploadResponse?.status && (
            <SuccessMessage successMessage={uploadResponse?.msg} />
          )}
          {uploadResponse?.msg && !uploadResponse?.status && (
            <ErrorMessage error={uploadResponse?.msg} />
          )}
